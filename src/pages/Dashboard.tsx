      ...
      <div className="relative z-10">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
            ...
            <section>
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-ink-400 flex items-center gap-2 text-sm font-semibold tracking-wider uppercase">
                  <FolderOpen className="h-4 w-4" /> Projects
                </h2>
                {projects && <span className="text-ink-500 text-xs">{projects.length} total</span>}
              </div>
              ...
              {projects === null ? (
                ...
                <div className="panel flex flex-col items-center justify-center px-6 py-16 text-center">
                  <div className="bg-brand-500/10 text-brand-400 ring-brand-500/20 flex h-14 w-14 items-center justify-center rounded-2xl ring-1">
                    <Plus className="h-6 w-6" />
                  </div>
                  <h3 className="mt-4 font-semibold">No projects yet</h3>
                  <p className="text-ink-400 mt-1 max-w-sm text-sm">Create your first minimap from a preset or import your existing tiles.</p>
                  ...
                  <AnimatePresence>
                    {projects.map((p, i) => (
                      <motion.div
                        ...
                        className="panel group hover:border-brand-500/40 relative overflow-hidden transition"
                      >
                        <Link to={`/editor/${p.id}`} className="block">
                          <div className="bg-ink-900 relative aspect-[4/3] overflow-hidden">
                            {p.thumbnail ? (
                              ...
                              <div className="grid-bg text-ink-600 flex h-full w-full items-center justify-center">
                        ...
                        <button
                          ...
                          className="bg-ink-950/80 text-ink-300 hover:text-ink-100 absolute top-2 right-2 rounded-lg p-1.5 backdrop-blur transition"
                        >
                        ...
                        <AnimatePresence>
                          {menuFor === p.id && (
                            ...
                            >
                              <button onClick={() => duplicate(p)} className="hover:bg-ink-700 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left">
            ...
            <aside className="space-y-4">
              <div className="panel p-5">
                <div className="flex items-center justify-between">
                  <h3 className="flex items-center gap-2 font-semibold">
                    <Sparkles className="text-brand-400 h-4 w-4" /> {plan.name} plan
                  </h3>
                  <span className="text-ink-500 text-xs">{plan.price}</span>
                </div>
              ...
              <div className="panel p-5">
                <h3 className="flex items-center gap-2 font-semibold">
                  <Clock className="text-ink-400 h-4 w-4" /> History
                </h3>
                {history.length === 0 ? (
                  <p className="text-ink-500 mt-3 text-sm">Your activity will show up here.</p>
                ) : (
                  <ul className="mt-3 max-h-80 scrollbar-thin space-y-2 overflow-y-auto pr-1">
                    {history.slice(0, 30).map((h) => (
                      <li key={h.id} className="flex items-start gap-2.5 text-sm">
                        ...
                        {h.action === 'exported' && <Download className="text-ink-500 ml-auto h-3.5 w-3.5 shrink-0" />}
                      </li>
                    ))}
                  </ul>
                )}
        ...
        <Modal open={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Delete project?" size="sm">
          <p className="text-ink-300 text-sm">
            <span className="text-ink-100 font-semibold">{confirmDelete?.name}</span> will be permanently removed. This cannot be undone.
          </p>
        ...
        <Modal
          ...
          description="Unlimited exports and 4K tiles for your server."
          size="sm"
        >
          <ul className="space-y-2 text-sm">
            {PLANS.supporter.features.map((f) => (
              <li key={f} className="text-ink-200 flex items-center gap-2">
                <Zap className="text-brand-400 h-3.5 w-3.5" /> {f}
              </li>
            ))}
          </ul>
          <p className="border-ink-700 bg-ink-900/60 text-ink-400 mt-4 rounded-lg border px-3 py-2 text-xs">
